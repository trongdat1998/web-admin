<template lang="pug">
  div.page-exchange
    ...
    undo(:data="listData", :loadingTable="loading" :productType="searchForm.productType" v-if="type==1" @searchData="loadData")
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchForm: {
          productType: 1, // 产品类型:0=定期 2=定期锁仓 1=活期
          pageNo: 1,
          size: 100,
        },
        loading: false,
    ...
    methods: {
      change(type) {
        this.$router.replace("/staking/rebate/current/" + type);
        this.search();
      },
      search() {
        this.listData = [];
