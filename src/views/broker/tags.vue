...
Main.page-exchange
  ...
  Modal(
    ...
    Form(:label-width="200")
      ...
      Alert
        Checkbox(
          v-for="(item, index) in localeDetail",
          v-model="item.enable",
          :key="index"
        ) {{ lang[item.language] }}
      template(v-for="item in localeDetail", v-if="item.enable")
        FormItem(:label="`${$lang('标签名称')}(${lang[item.language]})`")
          Input(
            ...
            :placeholder="`${$lang('标签名称')}(${lang[item.language]})`"
          )
            div(slot="append")
              span.border(
                :style="objectStyle(formData)",
                v-if="item.labelValue"
              ) {{ item.labelValue }}
...
export default {
  ...
  methods: {
    ...
    remove() {
      this.loading = true;
      this.$axios
        .post("/api/v1/symbol_custom_label/del", this.removeData)
        .then((result) => {
          this.loading = false;
          if (result.code == 0) {
    ...
    submit() {
      ...
      this.$axios
        .post("/api/v1/symbol_custom_label/save", data)
        .then((result) => {
          this.loading = false;
          if (result.code == 0) {
            this.creatWin = false;
    ...
    loadData() {
      ...
      this.$axios
        .post("/api/v1/symbol_custom_label/query", this.searchForm)
        .then((result) => {
