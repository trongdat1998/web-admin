...
Main.page-exchange
  Row.search(:gutter="15")
    ...
    Col(span="4")
      Select(v-model="searchForm.level" @on-change="loadData")
        Option(:value="2") {{$lang('二级待审核')}}
        Option(:value="3") {{$lang('三级待审核')}}
    Col(span="8")
      Input(:placeholder="`${$lang('用户ID')} / ${$lang('用户手机')} / ${$lang('用户邮箱')}`" v-model="inputInfo", search enter-button="" @on-search="loadData")
    Col(span="4")
      Button(long  type="primary" @click="batchVerify" :disabled="isRead" ghost) {{$lang("批量审核")}}
    ...
    Row(:gutter="15" v-show="!fromData.verifyPassed")
      ...
      Col(span="24" style="overflow:hidden")
        RadioGroup(v-model="fromData.reasonId")
          ...
          br
          Radio(border :label="0" style="width:100%") {{$lang("自定义回复")}}
            Input(v-model="remark", :placeholder="$lang('请填写对应语言的拒绝原因')" :disabled="fromData.reasonId !== 0" size="small" style="margin-bottom: 3px;")
...
export default {
  ...
  methods: {
    ...
    doNext() {
      ...
      if(this.fromData.reasonId == 0){
        Object.assign(this.fromData, {
          verifyPassed: false,
          reasonId: this.fromData.reasonId,
          remark: this.remark,
        });
      }
      this.$axios
