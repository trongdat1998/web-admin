...
;(_Object.prototype = {
  ...
  // v2版本
  (Config.prototype = {
    api_server: 'dkapi.geetest.com',
    type_path: '/deepknow/v2/gettype',
    protocol: 'https://',
    https: !0,
...
var isNumber = function(value) {
  ...
  jsonp = function(domains, path, config, callback) {
    ...
    ;(window[cb] = function(data) {
      callback('success' === data.status ? data.data : data),
        (window[cb] = undefined)
      try {
        delete window[cb]
      } catch (e) {
    ...
    return (
      ...
      userConfig && userConfig.id
        ? callback && !isFunction(callback)
          ...
          : void jsonp([config.api_server], config.type_path, config, function(
              newConfig
            ) {
              load(
                config.protocol,
                newConfig.static_servers,
                newConfig.path,
                null,
                function(err) {
